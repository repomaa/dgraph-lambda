...
import {
  GraphQLEvent,
  GraphQLEventWithParent,
  GraphQLEventFields,
  ResolverResponse,
  AuthHeaderField,
  WebHookGraphQLEvent,
} from "@slash-graphql/lambda-types";
...
class GraphQLResolverEventTarget extends EventTarget {
  addMultiParentGraphQLResolvers(resolvers: {
  ...
  addGraphQLResolvers(resolvers: {
    [key: string]: (e: GraphQLEventWithParent) => any | Promise<any>;
  }) {
    for (const [name, resolver] of Object.entries(resolvers)) {
      this.addEventListener(name, (e) => {
        const event = (e as unknown) as GraphQLEvent;
        event.respondWith(
          getParents(event).map((parent) => resolver({ ...event, parent }))
        );
      });
    }
  }

  addWebHookResolvers(resolvers: {
    [key: string]: (e: WebHookGraphQLEvent) => any | Promise<any>;
  }) {
    for (const [name, resolver] of Object.entries(resolvers)) {
      this.addEventListener(name, (e) => {
...
function newContext(eventTarget: GraphQLResolverEventTarget) {
  ...
  return vm.createContext({
    ...
    addMultiParentGraphQLResolvers: eventTarget.addMultiParentGraphQLResolvers.bind(
      eventTarget
    ),
...
export function evaluateScript(source: string) {
  ...
  return async function (e: GraphQLEventFields): Promise<any | undefined> {
    ...
    const event = {
      ...e,
      respondWith: (x: ResolverResponse) => {
        retPromise = x;
      },
      graphql: (
        query: string,
        ...
        ah?: AuthHeaderField
      ) => graphql(query, variables, ah || e.authHeader),
      dql,
    };
    ...
    if (
      ...
      resolvedArray.length !== getParents(e).length
    ) {
      process.env.NODE_ENV != "test" &&
        e.type !== "$webhook" &&
        console.error(
          `Value returned from ${e.type} was not an array or of incorrect length`
        );
      return undefined;
    }
